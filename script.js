...
// (II). Game Functions -> main function: ctime-currentTime in second
function main(ctime) {
...
function isSnakeCollide(snake) {
  ...
  for (let i = 1; i < snakeArr.length; i++) {
    // If x & y coordinates of 'snake head(snake[0])' and 'any one part of snake(snake[i])' is equal then sanke is dead
    if (snake[i].x === snake[0].x && snake[i].y === snake[0].y) {
      return true;
    }
  }
  // Snake bump into the wall :- If snake head is out of the board length then Snake is dead
  if (
    snake[0].x >= 36 ||
    snake[0].x <= 0 ||
    snake[0].y >= 36 ||
...
function isHighScore() {
  ...
  restartBtn.addEventListener("click", () => {
    // Window ko phir se reload karne ke liye
...
function gameEngine() {
  ...
  // When snake will eat the food(When x & y coordinates of 'snake head(snakeArr[0])' and food ) then we have to increment in score and regenerate the food
  if (snakeArr[0].y === food.y && snakeArr[0].x === food.x) {
    ...
    scoreBox.innerHTML = "Your Score :" + score;
    foodSound.play();
    // After eating food,we have to add food into snake
    snakeArr.unshift({
    ...
    let b = 34;
    food = {
      x: Math.round(a + (b-a+1) * Math.random()),
      y: Math.round(a + (b-a+1) * Math.random()),
    };
  }
  ...
  snakeArr.forEach((e, index) => {
    ...
    // board is type of 'grid'
    snakeElement.style.gridRowStart = e.y;
    snakeElement.style.gridColumnStart = e.x;

    // For Designing of Snake
    if (index === 0) {
...
// Touch Screen;
snakeBoard.addEventListener('click',(element)=>{
  let boardHeight=snakeBoard.offsetHeight;
  let boardWidth=snakeBoard.offsetWidth;
  console.log(snakeArr[0].x+" "+snakeArr[0].y);
  if(snakeArr.length===1){
    if(element.offsetY<=(boardHeight/2)){
      inputDir.x = 0;
      inputDir.y = -1;
    }else{
      inputDir.x = 0;
  ...
  if((snakeArr[0].x>snakeArr[1].x || snakeArr[0].x<snakeArr[1].x)&&(snakeArr[0].y===snakeArr[1].y)){
    if(element.offsetY<=(boardHeight/2)){
      inputDir.x = 0;
      inputDir.y = -1;
    }else{
  ...
  if((snakeArr[0].y>snakeArr[1].y || snakeArr[0].y<snakeArr[1].y)&&(snakeArr[0].x===snakeArr[1].x)){
    if(element.offsetX<=(boardWidth/2)){
      inputDir.x = -1;
      inputDir.y = 0;
    }else{
      inputDir.x = 1;
